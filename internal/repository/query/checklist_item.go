...
func (p *GetAllChecklistItemsQueryFunction) GetQueryFunction() func(connection pool.Conn) ([]dbo.ChecklistItemDbo, error) {
	return func(connection pool.Conn) ([]dbo.ChecklistItemDbo, error) {
		query := `				
			...
			WHERE (CAST(@checklist_item_completed AS BOOLEAN) IS NULL OR CHECKLIST_ITEM_COMPLETED = @checklist_item_completed)	
			AND CHECKLIST_ID = @checklist_id
			ORDER BY CHECKLIST_ITEM_COMPLETED ASC, ORDER_NUMBER ASC,
			CHECKLIST_ITEM_ROW_COMPLETED ASC`
