...
func (controller *templateController) AssignTemplateToWorkspace(ctx context.Context, request AssignTemplateToWorkspaceRequestObject) (AssignTemplateToWorkspaceResponseObject, error) {
	domainContext := serverutils.CreateContext(ctx)
	if err := controller.service.AssignTemplateToWorkspace(domainContext, request.TemplateId, request.Body.WorkspaceId); err == nil {
		return AssignTemplateToWorkspace204Response{}, nil
	} else if err.ResponseCode() == http.StatusNotFound {
		return AssignTemplateToWorkspace404JSONResponse{ErrorResponseJSONResponse: ErrorResponseJSONResponse{Message: err.Error()}}, nil
	} else {
		return AssignTemplateToWorkspace500JSONResponse{Message: err.Error()}, nil
	}
}

func (controller *templateController) UnassignTemplateFromWorkspace(ctx context.Context, request UnassignTemplateFromWorkspaceRequestObject) (UnassignTemplateFromWorkspaceResponseObject, error) {
	domainContext := serverutils.CreateContext(ctx)
	if err := controller.service.UnassignTemplateFromWorkspace(domainContext, request.TemplateId, request.WorkspaceId); err == nil {
		return UnassignTemplateFromWorkspace204Response{}, nil
	} else if err.ResponseCode() == http.StatusNotFound {
		return UnassignTemplateFromWorkspace404JSONResponse{ErrorResponseJSONResponse: ErrorResponseJSONResponse{Message: err.Error()}}, nil
	} else {
		return UnassignTemplateFromWorkspace500JSONResponse{Message: err.Error()}, nil
	}
}
